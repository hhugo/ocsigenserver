...
(* General config *)
let config_file = ref "/usr/local/etc/ocsigenserver.conf"
let verbose = ref false
let silent = ref false
let daemon = ref false
let veryverbose = ref false
let version_number = "2.2.0"
let mimefile = ref "/usr/local/etc/mime.types"
let pidfile : string option ref = ref None
...
let builtin_packages =
  List.fold_left (fun a s -> String.Set.add s a) String.Set.empty []

module Preempt = Lwt_preemptive
(* #else *)
(* module Preempt = *)
(* struct *)
...
let logdir = ref (Some ("/usr/local/var"))
let syslog_facility : Lwt_log.syslog_facility option ref = ref None
let default_user = ref "www"
let default_group = ref "www"
...
let datadir = ref "/usr/local/var"
let bindir = ref "/usr/local/bin"
let extdir = ref "/usr/local/lib"
let user = ref (Some !default_user)
let group = ref (Some !default_group)
let command_pipe = ref ("/usr/local/var" ^ "/ocsigen_command")
...
let set_max_number_of_threads_queued =
  Preempt.set_max_number_of_threads_queued
...
let get_max_number_of_threads_queued =
  Preempt.get_max_number_of_threads_queued
